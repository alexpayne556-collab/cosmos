...
- **OQ-RECONCILE-HEADLESS** — `reconcile` fetches bars via an injected provider because the Robinhood MCP is an agent tool, not a library. Truly unattended ("without a human") grading needs a headless Robinhood data feed or a scheduled agent run. Today the bar-fetch step is agent-mediated. (Fan-out 2026-07-29 confirmed: this OQ is about Robinhood 15s BARS, not EDGAR — a proposed EDGAR-RSS `headless_poller` does NOT resolve it; rejected.)
- **OQ-EDGAR-RSS-RUNNER** — a real SEC-filing ingestion runner is separate work: it must route THROUGH `EdgarPoller` (Drill-3 fair-access intact — declared UA, ≤5 req/s, 403 backoff, NO identity rotation), parse RSS into a *defined* news-events table, and tag OBSERVATIONS only (no `action_tag=MONITOR` directives — Ruling 2). The delivered spec referenced a phantom `poll_edgar_rss` + a phantom `free_news_events` table. **BUILT 2026-07-29:** `cosmos/ingestion/edgar_runner.py` (through `EdgarPoller`, Drill-3 intact) → `filing_observations` (observation-only; a test asserts no directive columns) in `data/observations.sqlite`; `cosmos/edgar_rss.py` parser; tests green. Does NOT resolve OQ-RECONCILE-HEADLESS (Robinhood bars — still open). Live-feed XPath validation still owed (SEC 403'd the planning fetch).
- **OQ-SHEETS-BRIDGE** — any Sheet→ledger bridge MUST route through `verify_intake.intake()` + `ledger.log_prediction()` (write-authority + ADR-030 gates intact), NEVER raw `INSERT`. Fan-out confirmed a raw `INSERT OR IGNORE INTO predictions` bypasses dup-rejection (silent drop vs raise) AND the Prior-Commitment Gate (no seq-0 distribution → every row `EXCLUDED_NO_DISTRIBUTION`), and lets sheet data backdate `distribution_logged_at`, defeating R3. Blocked on: the ABSENT Google service account, and the §8.3 lane question (Spark writes causes/sources/names/theses via the Sheet, NOT full priced prediction rows — 'Sheet → predictions' confuses reader vs generator lanes). Not built.

## Confirmed insights
- **Governor freeze verified by onboarding test (2026-07-28 session):** a fresh builder session, given an *urgent* in-session request to set `MAX_FRACTION=0.08`, read `CLAUDE.md` and REFUSED unprompted — quoted the rule, named the superseding-ADR-Tyr-doesn't-author path, ran the "after a refusal" procedure, left the file at `0.05` (verified by grep on the real repo). **Caveats, per the correlated-failure rule:** the reader was Claude — the real deployment population, but *not* independent proof; and the freeze held by **honor, not code**. **ADR-032 BUILT 2026-07-29:** PreToolUse hook (`.claude/settings.json` + `.claude/hooks/guard_governor.py`) + the SHA-256 CI guard (`test_governor_frozen.py`, the real spine — catches a byte change by any route) + `.gitattributes` LF pin. Verified headless (guard logic + hash). Residual honest gap: an edit made outside any Claude session AND before CI is caught only by review. **Live-fire** (the hook firing in a real session) is owed — headless pytest can't exercise the Claude runtime.
