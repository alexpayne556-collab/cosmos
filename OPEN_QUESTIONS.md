...
- **OQ-GENESIS-DISTRIBUTIONS** — the ten Section-5 rows carry NO generator distribution, so the live grading pass resolves outcomes but cannot score Brier or move a weight on them. The verify station will not fabricate a generator's probability vector (ADR-002). To score the live book, `claude` and `gemini_spark` must supply a `{up,down,no_move}` distribution per row. This is the one thing blocking a live Brier number.
...
- **Loop closed at the mechanism level 2026-07-28:** `test_full_loop_closes` runs write→measure→grade→Brier(0.26)→weight-change end-to-end; the live pass graded AMKR on real bars. Remaining to reach fully-autonomous live Brier: OQ-GENESIS-DISTRIBUTIONS + OQ-RECONCILE-HEADLESS.
