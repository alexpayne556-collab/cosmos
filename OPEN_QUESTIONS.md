...
- **OQ-GENESIS-DISTRIBUTIONS** — PARTIALLY RESOLVED 2026-07-28. Orchestrator delivered distributions in the direction-agnostic schema `{hit_target_first, hit_invalidation_first, expire_in_range}` (adopted; Brier now scored in bucket space, `RULE_TO_BUCKET`). **6 of 10 rows covered** (AMKR/APLD/BA/PYPL/KO/STX) → attached in `ledger.GENESIS_DISTRIBUTIONS`. First live Brier landed: **AMKR 0.892**, `claude` weight 0.500→0.505.
- **OQ-GENESIS-DISTRIBUTIONS-MISSING4** — **CLS / NE / CDNS / SMH** (all Claude rows) still have NO distribution → they resolve outcomes but score no Brier. Their generator (`claude`) must supply them; the verify station will not fabricate them (ADR-002).
- **OQ-GENESIS-DISTRIBUTIONS-ORPHANS** — 14 distributions in the same drop attach to NO prediction row (board names FRO/INSW/KTOS/ASTS/RKLB/LUNR/AEM/FNV/NEM + UHS/NXPI/KLAC/ENPH/F). Retained as `ledger.UNATTACHED_PRIORS`; they score nothing until a belief exists. Many vectors are identical `0.58/0.27/0.15` — plausible templated priors, not differentiated per-belief forecasts (worth revisiting).
...
- **Loop closed on a LIVE prediction 2026-07-28:** the grading pass wrote→measured→graded AMKR to a **real Brier 0.892** and moved `claude`'s weight 0.500→0.505 (logged). `test_full_loop_closes` also proves the cycle end-to-end. Remaining to reach fully-UNATTENDED live Brier: OQ-GENESIS-DISTRIBUTIONS-MISSING4 (4 rows) + OQ-RECONCILE-HEADLESS (agent-mediated bar fetch).
