...
def test_genesis_import_idempotent_and_complete():
    ...
    assert json.loads(amkr[3])["hit_target_first"] == 0.58   # ratified distribution attached
    # the four uncovered rows still carry NO distribution (not fabricated)
    cls = con.execute("SELECT distribution FROM predictions WHERE ticker='CLS'").fetchone()
    assert cls[0] is None
